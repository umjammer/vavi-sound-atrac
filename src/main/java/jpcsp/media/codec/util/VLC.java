...
public class VLC {
    ...
    private static final Logger logger = getLogger(VLC.class.getName());
    ...
    public int initVLCSparse(int nbBits, int nbCodes, int[] bits, int[] codes, int[] symbols) {
        ...
        for (int i = 0; i < nbCodes; i++) {
            ...
            if (buf[j].bits > 3 * nbBits || buf[j].bits > 32) {
                logger.log(Level.ERROR, String.format("Too long VLC (%d) in initVLC", buf[j].bits));
                return -1;
            }
            buf[j].code = codes[i];
            if (buf[j].code >= (1 << buf[j].bits)) {
                logger.log(Level.ERROR, "Invalid code in initVLC");
                return -1;
            }
        ...
        for (int i = 0; i < nbCodes; i++) {
            ...
            if (n <= tableNbBits) {
                ...
                for (int k = 0; k < nb; k++) {
                    ...
                    if (bits != 0 && bits != n) {
                        logger.log(Level.ERROR, "incorrect codes");
                        return -1;
                    }
